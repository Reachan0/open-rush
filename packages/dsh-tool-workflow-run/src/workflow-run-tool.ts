...
export async function runWorkflowFromLoop(input: {
  ...
  const rawLoop = createLoopToolInvoker({
    ...
    callIdPrefix: `${String(input.exec?.callId ?? 'wf')}:wf`,
  });
  ...
  return defineTool({
    ...
    parameters: {
      ...
      dsl: {
        type: 'string',
        required: false,
        description: '可选。已有 JSON 图则只执行、不再规划。',
      },
    },
    output: {
      schema: {
...
export function buildWorkflowRunPromptSection(schemas: LoopToolSchema[] = []): string {
  const eligible = formatEligibleLoopToolList(schemas);
  const denied = formatDeniedLoopToolList(schemas);
  const live =
    eligibleLoopTools(schemas).length > 0
