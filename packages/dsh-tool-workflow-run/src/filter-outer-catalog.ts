...
export function isOuterLoopTool(
  name: string,
  options?: { eligibleToolNames?: readonly string[] }
): boolean {
  return (
    name === WORKFLOW_RUN_TOOL_NAME ||
    !isEligibleLoopTool(name, { additionalEligibleTools: options?.eligibleToolNames })
  );
}
...
export function filterOuterLoopCatalog<T extends OuterCatalogAssembly>(
  ...
  assembly.tools = assembly.tools.filter((tool) => !shouldHide(tool.name));
  return assembly;
}
// AIGC END
