...
describe('isDeniedLoopTool', () => {
  ...
  it('does not let unknown or mutating MCP into the graph', () => {
    ...
    expect(isEligibleLoopTool('ao04_read_status')).toBe(false);
    expect(
      isEligibleLoopTool('ao04_read_status', { additionalEligibleTools: ['ao04_read_status'] })
    ).toBe(true);
    expect(
      isEligibleLoopTool('mcp__unknown__delete_record', {
        additionalEligibleTools: ['mcp__unknown__delete_record'],
      })
    ).toBe(false);
  });
  ...
  it('lists shareable Loop tools and keeps denylist reasons for the outer prompt', () => {
    ...
    expect(
      formatEligibleLoopToolList(
        [{ name: 'ao04_read_status', description: 'Read protected status' }],
        { additionalEligibleTools: ['ao04_read_status'] }
      )
    ).toContain('ao04_read_status');
  });

  it('only adds explicitly configured runtime tools to the graph catalog', () => {
    expect(schemasToToolDescriptors([{ name: 'ao04_read_status' }])).toEqual([]);
    expect(
      schemasToToolDescriptors([{ name: 'ao04_read_status' }], {
        additionalEligibleTools: ['ao04_read_status'],
      })
    ).toEqual([expect.objectContaining({ name: 'ao04_read_status' })]);
  });
});
...
describe('createLoopToolInvoker', () => {
  it('lists schemas minus the denylist and forwards execute arguments as-is', async () => {
    const execute = vi.fn(async (_input: { name: string; arguments: unknown }) => ({
      isError: false,
  ...
  it('logs tool lifecycle without arguments or returned error text', async () => {
    const writes: string[] = [];
    ...
    try {
      ...
      await tools.invoke('glob', { token: 'secret-token', glob_pattern: 'README.md' });
      const joined = writes.join('');
      expect(joined).toMatch(/\[workflow_run\] execute DSH tool: glob callId=/);
      expect(joined).toMatch(/\[workflow_run\] DSH tool ok: glob callId=.* durationMs=/);
  ...
  it('applies a named result adapter without changing ordinary tool values', async () => {
    const adapter = {
      success: (value: unknown) => ({ adapted: value }) as never,
    };
    const tools = createLoopToolInvoker({
      tools: {
        schemas: () => [{ name: 'ao04_read_status' }, { name: 'read' }],
        execute: async ({ name }) => ({
          isError: false,
          value: name === 'ao04_read_status' ? { text: '{"status":"ok"}' } : { text: 'plain' },
        }),
      },
      eligibleToolNames: ['ao04_read_status'],
      resultAdapters: { ao04_read_status: adapter },
    });
    expect((await tools.listTools()).map((tool) => tool.name)).toEqual([
      'ao04_read_status',
      'read',
    ]);
    await expect(tools.invoke('ao04_read_status', {})).resolves.toEqual({
      adapted: { text: '{"status":"ok"}' },
    });
    await expect(tools.invoke('read', {})).resolves.toEqual({ text: 'plain' });
  });

  it('keeps the protected tool call failure path available to adapters', async () => {
    const failure = {
      failure: ({ result }: { result?: { error?: { message?: string } } }) =>
        new WorkflowError(
          ...
          {
            fatal: true,
            details: { status: 'failed', evidenceRefs: ['e-1'] },
          }
        ),
    };
    const tools = createLoopToolInvoker({
      tools: {
        schemas: () => [{ name: 'ao04_read_status' }],
        execute: async () => ({ isError: true, error: { message: 'HTTP 503' } }),
      },
      eligibleToolNames: ['ao04_read_status'],
      resultAdapters: { ao04_read_status: failure },
    });
    await expect(tools.invoke('ao04_read_status', {})).rejects.toMatchObject({
      code: 'protected_tool_failed',
      fatal: true,
      details: { evidenceRefs: ['e-1'] },
    });
  });
});
// AIGC END
