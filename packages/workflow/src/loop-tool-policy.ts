...
export interface LoopToolPolicyOptions {
  /** Exact runtime names explicitly allowed by a composition. */
  additionalEligibleTools?: readonly string[];
}
...
export function loopToolDenyReason(
  name: string,
  ...
  if (!isEligibleLoopTool(name, options)) return '未知工具默认不进图，由外层慢车道使用';
  return undefined;
}

function isAdditionalEligibleTool(name: string, options?: LoopToolPolicyOptions): boolean {
  const normalized = name.trim().toLowerCase();
  return (options?.additionalEligibleTools ?? []).some(
    (candidate) => candidate.trim().toLowerCase() === normalized
  );
}

export function isEligibleLoopTool(name: string, options?: LoopToolPolicyOptions): boolean {
  const n = name.trim().toLowerCase();
  if (!n || isDeniedLoopTool(n)) return false;
  if (n.includes('apply_patch') || n.includes('exec_command') || DANGEROUS_STEM.test(n)) {
    return false;
  }
  if (isAdditionalEligibleTool(name, options)) return true;
...
export function eligibleLoopTools<T extends NamedLoopTool>(
  schemas: T[],
  options?: LoopToolPolicyOptions
): T[] {
  return schemas.filter((schema) => isEligibleLoopTool(schema.name, options));
}

export function deniedLoopTools<T extends NamedLoopTool>(
  schemas: T[],
  options?: LoopToolPolicyOptions
): T[] {
  return schemas.filter((schema) => !isEligibleLoopTool(schema.name, options));
...
export function formatEligibleLoopToolList(
  schemas: NamedLoopTool[],
  options?: LoopToolPolicyOptions
): string {
  const eligible = eligibleLoopTools(schemas, options);
...
export function formatDeniedLoopToolList(
  schemas: NamedLoopTool[],
  options?: LoopToolPolicyOptions
): string {
  const seen = new Set<string>();
  const lines: string[] = [];
  for (const schema of deniedLoopTools(schemas, options)) {
    const key = schema.name.trim().toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
    const reason = loopToolDenyReason(schema.name, options);
