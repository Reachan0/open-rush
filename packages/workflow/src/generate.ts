...
- web.search is live web search. Use it only when the user asked to 搜索/搜一下 the open web, or for non-workspace gather. If the user 搜一下 inside 工作区 / a repo and coding-tools__search_text is in the catalog, that is local search_text { query }, NOT web.search. Input.query must be a short search string.`;
...
export function buildGeneratePrompt(intent: string, tools: ToolDescriptor[]): string {
  ...
  if (/https?:\/\//i.test(intent) && noSearch) {
  ...
  } else if (workspaceInspect && (noSearch || catalogHasCoding(tools))) {
    hint = catalogHasCoding(tools)
      ...
      : `\nPlanning hint: workspace files. MUST fs.read each named path (package.json, .env.example, README.md, etc; parallel if multiple), then text.compose. Do NOT use web.search.\n`;
