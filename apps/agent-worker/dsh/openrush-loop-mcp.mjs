...
async function connectMcp(ctx, mcp, config) {
  ...
  process.stderr.write(
    `[openrush-loop-mcp] connecting ${config.serverName} (${config.transport})\n`
  );
  try {
...
export async function apply(ctx) {
  ...
  process.stderr.write(
    `[openrush-loop-mcp] startup ${Date.now() - started}ms (${jobs.length} servers)\n`
  );
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      codingOn
        ? '改文件、跑命令仍用外层 write / edit / bash；coding-tools 只读能力同样走 workflow_run。'
        : '改文件、跑命令仍用外层 write / edit / bash。',
    ].join(' '),
  });
}
// AIGC END
